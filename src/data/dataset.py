...
class LightningDataset(Dataset):
    ...
    def __init__(self,
        ...
        Args:
            ...
            target_cape_shape: Target shape for CAPE data (native 25km resolution)
            target_lightning_shape: Target shape for lightning data (3km resolution)
            target_terrain_shape: Target shape for terrain data (native 1km resolution)
        """
    ...
    def _build_sample_index(self):
        ...
        for file_idx, file_dict in enumerate(self.file_pairs):
            try:
                # Load CAPE file to get time dimension
                cape_ds = NetCDFHandler.load_netcdf(file_dict['cape'], variables=['cape'])
                lightning_ds = NetCDFHandler.load_netcdf(file_dict['lightning'], variables=['lightning_occurrence'])
                
                if cape_ds is None or lightning_ds is None:
                    logger.warning(f"Skipping file pair {file_idx}: failed to load data")
                    continue
                
                # Get time dimensions
                cape_times = len(cape_ds.time) if 'time' in cape_ds.dims else 1
                lightning_times = len(lightning_ds.time) if 'time' in lightning_ds.dims else 1
