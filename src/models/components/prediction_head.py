...
class PredictionHead(nn.Module):
    """
    Lightning prediction head that outputs raw logits for lightning occurrence.
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            activation: Final activation function (THIS IS NOW IGNORED for the main output)
        ...
        # The explicit activation function is no longer used in the forward pass
        # to avoid the double-sigmoid issue with BCEWithLogitsLoss.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Raw lightning logits (batch_size, output_dim, height, width)
            If use_uncertainty=True, returns tuple (logits, uncertainty)
        """
        # Process features
        processed_features = self.feature_processor(x)
        
        # Predict lightning logits
        lightning_logits = self.lightning_predictor(processed_features)
        
        # FIX: Removed the final activation function from the main prediction path.
        # The model must output raw logits to be compatible with `BCEWithLogitsLoss`.
        # OLD LINE: lightning_pred = self.activation(lightning_logits)
        
        if self.use_uncertainty:
            # Predict uncertainty
            uncertainty_logits = self.uncertainty_predictor(processed_features)
            # Uncertainty can still be a probability between 0 and 1 for interpretation
            uncertainty = torch.sigmoid(uncertainty_logits) 
            
            return lightning_logits, uncertainty # Return raw logits for the main prediction
        
        return lightning_logits # Return raw logits
...
class MultiScalePredictionHead(nn.Module):
    ...
    def __init__(self,
        ...
        # Each of these will now correctly output logits because PredictionHead was fixed
        self.scale_predictors = nn.ModuleList([
            PredictionHead(input_channels, hidden_dim, output_dim, activation="identity")
            for _ in scales
        ])
        ...
        self.scale_fusion = nn.Sequential(
            ...
            # FIX: Removed nn.Sigmoid() from the end of this sequence. 
            # The fusion network must also output logits.
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Make predictions (as logits) at each scale
        scale_predictions_logits = []
        for feat, predictor in zip(scale_features, self.scale_predictors):
            pred_logits = predictor(feat)
            scale_predictions_logits.append(pred_logits)
        
        # Compute attention weights for scales
        scale_weights = self.scale_attention(x)
        
        # Weighted combination of scale predictions (logits)
        weighted_preds_logits = sum(w.unsqueeze(2).unsqueeze(3) * pred 
                                   for w, pred in zip(scale_weights.split(1, dim=1), scale_predictions_logits))
        
        # Concatenate all predictions for fusion
        all_preds_logits = torch.cat(scale_predictions_logits, dim=1)
        
        # Final fusion (logits)
        fused_prediction_logits = self.scale_fusion(all_preds_logits)
        
        # Combine weighted and fused predictions (logits)
        final_prediction_logits = 0.7 * weighted_preds_logits + 0.3 * fused_prediction_logits
...
class PhysicsInformedHead(PredictionHead):
    ...
    def forward(self, x: torch.Tensor, cape_data: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # FIX: The inherited forward pass is not called, so we ensure no activation is applied here.
        # This method correctly returns raw logits.
        return lightning_logits
...
class EnsemblePredictionHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            Tuple of (ensemble_prediction_logits, prediction_uncertainty)
        """
        # Get predictions from all heads (as logits)
        predictions_logits = []
        for head in self.prediction_heads:
            pred = head(x)
            # Handle the case where the head returns (logits, uncertainty)
            if isinstance(pred, tuple):
                pred_logits = pred[0]
            else:
                pred_logits = pred
            predictions_logits.append(pred_logits)
        
        # Stack predictions
        all_preds_logits = torch.stack(predictions_logits, dim=0)  # (num_heads, batch, channels, h, w)
        
        # Compute ensemble weights (softmax normalized)
        weights = F.softmax(self.ensemble_weights, dim=0)
        
        # Weighted ensemble prediction (on logits)
        ensemble_pred_logits = torch.sum(weights.view(-1, 1, 1, 1, 1) * all_preds_logits, dim=0)
        
        # Compute uncertainty as prediction variance on probabilities
        all_preds_probs = torch.sigmoid(all_preds_logits)
        pred_variance = torch.var(all_preds_probs, dim=0)
